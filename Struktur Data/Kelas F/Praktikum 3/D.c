...
void _____map_inorder(AVLNode *root) {
    if (root) {
        ...
        printf("%d. %s\n", n++, DStringGetString(&root->key));
        _____map_inorder(root->right);
    }
}

void _____map_inordergg(AVLNode *root) {
    if (root) {
        _____map_inordergg(root->left);
        printf("%s", DStringGetString(&root->key));
        if (kk < n - 2){
            printf("_");
            kk++;
        }
        _____map_inordergg(root->right);
    }
}
...
// Dengan menggunakan ADT AVL, Map dapat disimulasikan
// Inisialisasi Map : map_init(&avl)
// Menambahkan pasangan key value : map_insert(&avl,"key","value")
// Modifikasi value dengan key yang ada : map_modify(&avl,"key","value")
// Menghapus pasangan key dan value dengan key yang ada : map_remove(&avl,"key")
// Mendapatkan value dengan key yang ada : map_get(&avl,"key")
// Print Map secara inorder : map_inorder(&avl)
// Print Map secara reversed inorder : map_reverse_inorder(&avl)
// Print Map secara preorder : map_preorder(&avl)
// Print Map secara postorder : map_postorder(&avl)
int main (){
    AVL lav;
